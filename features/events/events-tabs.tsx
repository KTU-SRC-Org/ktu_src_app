      ...
      {eventTabOptions.map((option) => {
        ...
        return (
          <Pressable
            ...
            className={`flex-1 items-center rounded-md py-3 ${active ? 'bg-neutral-800' : 'bg-neutral-200'}`}>
            <Text className={`font-semibold ${active && 'text-white'}`}>{option.label}</Text>
          </Pressable>
        );
      })}
    </View>
