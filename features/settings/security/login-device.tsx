      ...
      <ScrollView
        ...
        <View className="flex-col gap-4">
          <View className={'flex w-full items-center justify-center'}>
            <View className="h-16 w-16 items-center justify-center rounded-full border border-neutral-500 p-4">
              <DeviceIcon />
            </View>
          </View>
          ...
          <View className="flex-col gap-2">
            ...
            <DeviceCard device={devices.current} onPress={setSelectedDevice} />
          ...
          <View className="flex-col gap-2">
            ...
            <View className={'flex-col gap-4'}>
              {devices.active.map((device) => (
                <DeviceCard key={device.id} device={device} onPress={setSelectedDevice} />
              ))}
            </View>
          </View>
          <Text className="text-center text-sm text-neutral-500">
            Your sessions help keep your SRC account secure across all logged-in devices. Manage
            your activity to ensure your information stays protected.
          </Text>
        </View>
      </ScrollView>
      ...
      <FormModal
        ...
        <View className="w-full flex-col items-start gap-4">
          <View className="w-full flex-col items-start gap-2">
            ...
            <Text className="text-2xl font-bold">{selectedDevice?.name}</Text>

            <DetailRow label="Last Active" value={selectedDevice?.lastActive} />
            <DetailRow label="Application" value={selectedDevice?.application} />
            <DetailRow label="Location" value={selectedDevice?.location} showBorder={false} />
...
const SectionHeader = ({ title }: SectionHeaderProps) => (
  <Text className="text-xs font-semibold uppercase tracking-wide">{title}</Text>
);
